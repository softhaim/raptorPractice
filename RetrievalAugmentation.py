...
class RetrievalAugmentationConfig:
    def __init__(
        ...
        if qa_model is not None and not isinstance(qa_model, BaseQAModel):
            ...
            qa_model = LLaMAQAModel()  # LLaMA QAModel로 설정
        ...
        if embedding_model is not None and not isinstance(embedding_model, BaseEmbeddingModel):
            ...
            tb_cluster_embedding_model = embedding_model  # 실제 LLaMA 인스턴스를 사용
            tr_context_embedding_model = embedding_model  # 실제 LLaMA 인스턴스를 사용
        ...
        elif summarization_model is None:
...
class RetrievalAugmentation:
    ...
    def answer_question(
        ...
        # Use the LLaMA-based QA model to answer the question based on the retrieved context
